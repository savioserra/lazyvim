...
- The command runs in the foreground. Inactive systemd-user and LaunchAgent assets are repository managed, but lifecycle never loads or starts them and both activation gates remain false. GoAkt v4.5.2 cannot enforce the required inbound CIDR-to-mTLS-identity binding, so enabled remoting fails startup and production never installs `actor.WithRemote`.
