...
func identityBridgeAck(agentID, runtimeID, piSessionID string, incarnation uint64, delivery *subagentsv1.BridgeDelivery, delivered bool, result []byte) *subagentsv1.BridgeDeliveryAckRequest {
	return &subagentsv1.BridgeDeliveryAckRequest{AgentId: agentID, Sequence: delivery.Sequence, DedupeId: delivery.DedupeId, Delivered: delivered, BoundedResult: result, RuntimeId: runtimeID, Incarnation: incarnation, PiSessionId: piSessionID, Kind: deliveryKindLabel(delivery.Kind), SourceScope: delivery.SourceScope, CompletionKey: delivery.CompletionKey}
}
