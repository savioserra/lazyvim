...
if ($env:OS -ne 'Windows_NT') {
    throw 'scripts/install-windows.ps1 only supports Windows; use scripts/install-linux or scripts/install-macos.'
}
...
function Install-ZipDirectory {
    ...
    $releaseIdentity = "${Platform}:$Sha256"
    if (Test-ReleaseInstallation `
        -Target $target `
        -ExpectedBinary $ExpectedBinary `
        -ReleaseIdentity $releaseIdentity) {
        Write-DotfilesLog "Already installed: $Name $Version"
        return $target
    }
    ...
    Write-ReleaseIdentity -Target $target -ReleaseIdentity $releaseIdentity
    Write-DotfilesLog "Installed $Name $Version"
    return $target
}
...
if ($InstallFont) {
    ...
    if ((Test-Path -LiteralPath $fontMarker -PathType Leaf) -and (Test-Path -LiteralPath $regularFont -PathType Leaf)) {
        Write-DotfilesLog "Already installed: JetBrainsMono Nerd Font $($Tools.NERD_FONT_VERSION)"
    }
    else {
        ...
        Write-DotfilesLog "Installed JetBrainsMono Nerd Font $($Tools.NERD_FONT_VERSION)"
    }
}
...
Write-DotfilesLog "Installation complete for $Platform"
