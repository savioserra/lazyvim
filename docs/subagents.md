...
Tailscale ACL/device identity is the approved network boundary; mTLS is an independent actor-plane allowlist. GoAkt v4.5.2 cannot bind one URI SAN to one source IP, so every configured peer is equally trusted. The initial topology lists three nodes, keeps two replicas, requires both replicas for writes, and allows reads from one; a node needs at least one peer to bootstrap. An isolated node therefore cannot accept cluster-registry writes. Automatic actor relocation is disabled: DNS changes and node loss cannot migrate a hosted actor's durable home authority. Public cross-node AgentActor placement is explicitly typed: a caller must hold `admin` before node lookup/remote spawn and `observe`/`tell`/`ask` before lookup/routing, private actors are excluded from the public projection, and routing uses the recorded durable home-node endpoint rather than relocating actors. Non-protobuf actor-plane messages use an explicit append-only CBOR serializer registry. Managed configuration remains off until certificates and physical VPS/macOS validation are complete.
