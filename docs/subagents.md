...
bind_host = "aurora.example.internal"
magicdns_suffix = ".example.internal"
port = 17210
...
4. Run `/actor-create e2e-worker -- <absolute scratch worktree> -- E2E Worker -- reviewer`, `/actor-ask e2e-worker -- Reply exactly E2E_OK`, and confirm the correlated answer is `E2E_OK` with no secret/credential path output. Restart Pi and repeat `/actor-list` and one `/actor-send` to prove bounded reconnect/readiness.
