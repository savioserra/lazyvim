...
A user-launched terminal Pi uses the ordinary actor-client extension and authenticated WebSocket only to attach to its independent terminal `AgentActor`; that actor's ActorRef is the source peer. A hosted Pi uses the repository-managed hosted bridge extension over the same application endpoint as execution infrastructure for its owning hosted AgentActor. The hosted bridge binds exactly one runtime ID/incarnation and owned Pi session to one authenticated hosted AgentActor handle/fence. The registry validates the current session instance, generation, credential, caller, capability, deadline, and target before resolving the actor with `ActorOf`. The gateway commands the authenticated source AgentActor, which uses direct GoAkt Tell to the target; target completion returns by Tell to the original source ActorRef. An omitted human-command target is normalized only by the hosted bridge to its hosting AgentActor; model tools and terminal Pi peer routing use explicit authorized targets.
