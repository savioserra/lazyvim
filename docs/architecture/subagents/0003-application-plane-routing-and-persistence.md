...
Public targets are stable logical agent IDs or opaque capability handles, never raw PIDs. The registry maps a logical agent to its durable home node; public routing checks session credential/capability before any actor lookup, excludes private actors from public discovery, rejects stale home nodes, and then uses GoAkt remote lookup/Tell/Ask against the recorded endpoint. Typed remote placement creates only explicitly registered public AgentActor kinds on an explicitly named node with relocation disabled; hosted actors are not remotely relocated or shell-driven. Requests carry a typed payload, request ID, absolute deadline, required capability, mutation operation identity, hop limit, and dedupe identity. Hop limits prevent bridge/routing loops. Ask responses are typed success/failure values. Pi TUI commands and model/tool calls receive ordinary typed tool responses; the bridge does not synthesize terminal input.
