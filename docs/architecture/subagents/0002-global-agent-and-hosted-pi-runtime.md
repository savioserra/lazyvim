...
Tmux ownership is exact: tmux-server PID/start-token, captured stable session/window/pane IDs, human-readable names, pane PID/start-token, and TTY are recorded before readiness. Cleanup and startup rollback each use one tmux client and one server-serialized `if-shell -F` queue that checks the exact marked tuple on that connected server immediately before killing the stable session ID; no reconnect-to-kill occurs. The owned pane enables `remain-on-exit` so unexpected termination retains bounded exit status/signal evidence until that same exact cleanup queue removes the dead session. A proven-clean unexpected exit is retried with bounded exponential backoff and a new incarnation; indeterminate ownership is never respawned. Tests use a separate tmux server name and config and prove a foreign session survives.
