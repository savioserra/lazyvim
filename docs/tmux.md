...
TPM's `'user/repo#<ref>'` syntax only accepts branches/tags, not raw SHAs. Pinning is done by `home/dot_local/share/lazyvim/setup.mjs`, with the source-of-truth list in `lib/tmux.mjs`; `dot_tmux.conf`'s `@plugin` lines stay bare `user/repo`.
