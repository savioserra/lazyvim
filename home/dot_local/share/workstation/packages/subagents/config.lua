...
function M.verify_inactive(contents)
	...
	assert(
		tables.service and service_enabled == "false",
		"repository-managed subagents [service].enabled must be false"
	)
	assert(
		tables.hosted_pi and hosted_pi_enabled == "false",
		"repository-managed subagents [hosted_pi].enabled must be false"
	)
	local _, _, remoting = flags(contents)
	assert(not remoting, "repository-managed subagents [remoting].enabled must be false")
end
