...
export function registerClientHandlers(api:API,operations:ClientOperations,schemas:{empty:unknown;target:unknown;actorTarget:unknown;actorTask:unknown;task:unknown;lifecycle:unknown;create:unknown}){
 ...
 api.registerCommand("actor-create",{description:"Create a hosted actor: name -- /absolute/worktree [-- display name] [-- dynamic role]",handler:async(args,ctx)=>{const[a,p,d,r]=parseCreateArgs(args);notify(ctx,"actor_create",await operations.create(a,p,d,r));}});
