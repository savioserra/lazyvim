...
export function registerClientHandlers(api:API,operations:ClientOperations,schemas:{empty:unknown;target:unknown;actorTarget:unknown;actorTask:unknown;create:unknown}){
 ...
 api.registerCommand("actor-tell",{description:"Tell an actor asynchronously: actor -- message",handler:async(args,ctx)=>{const[a,p]=parsePair(args);notify(ctx,"actor_tell",await operations.ask(a,p));}});
