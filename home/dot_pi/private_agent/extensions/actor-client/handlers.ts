...
export function registerClientHandlers(api:API,operations:ClientOperations,schemas:{empty:unknown;target:unknown;actorTarget:unknown;actorTask:unknown;task:unknown;lifecycle:unknown;create:unknown}){
 ...
 api.registerCommand("actor-client-create",{description:"Create a client actor: name -- /absolute/worktree [-- display name] [-- dynamic role] [-- node identity]",handler:async(args,ctx)=>{const[a,p,d,r,n]=parseCreateArgs(args);notify(ctx,"actor_client_create",await operations.create(a,p,d,r,n));}});
