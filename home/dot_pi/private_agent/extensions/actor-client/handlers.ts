...
export function registerClientHandlers(api:API,operations:ClientOperations,schemas:{empty:unknown;target:unknown;actorTarget:unknown;actorTask:unknown;task:unknown;lifecycle:unknown;create:unknown}){
 ...
 api.registerCommand("actor-send",{description:"Send a typed notification: actor -- message",handler:async(args,_ctx)=>{const[a,p]=parsePair(args);await operations.send(a,p);}});
 api.registerCommand("actor-ask",{description:"Run a real model task: actor -- prompt",handler:async(args,_ctx)=>{const[a,p]=parsePair(args);await operations.ask(a,p);}});
 api.registerCommand("actor-prompt-start",{description:"Start an observed model task: actor -- prompt",handler:async(args,_ctx)=>{const[a,p]=parsePair(args);await operations.promptStart(a,p);}});
 api.registerCommand("actor-prompt-status",{description:"Read observed task status: actor -- lifecycle-id",handler:async(args,_ctx)=>{const[a,p]=parsePair(args);await operations.promptStatus(a,p);}});
 api.registerCommand("actor-prompt-wait",{description:"Wait briefly for observed task completion: actor -- lifecycle-id",handler:async(args,_ctx)=>{const[a,p]=parsePair(args);await operations.promptWait(a,p);}});
 api.registerCommand("actor-status",{description:"Show actor status",handler:async(args,ctx)=>notify(ctx,"actor_status",await operations.status(args.trim()))});
