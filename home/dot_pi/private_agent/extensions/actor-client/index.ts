...
export default async function wsActorClient(pi: ExtensionAPI) {
  ...
  registerClientHandlers(pi as any,{create:createActor,list,resolve,send,ask,promptStart,promptStatus:promptObserve,promptWait,status,attach:attachCommand,stop,subscribe,unsubscribe,control},{empty:Type.Object({}),target,actorTarget,actorTask,task,lifecycle,create:Type.Object({agentId:Type.String({minLength:1}),projectDirectory:Type.String({minLength:1}),displayName:Type.Optional(Type.String({minLength:1,maxLength:80})),role:Type.Optional(Type.String({minLength:1,maxLength:64}))})});
