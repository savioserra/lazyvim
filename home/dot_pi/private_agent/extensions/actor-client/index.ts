...
class Client {
  ...
  async close() { const socket = this.socket;this.socket=undefined;if(!socket)return;if(socket.readyState===WebSocket.CLOSED)return;socket.close();await new Promise<void>((resolve)=>{const timer=setTimeout(resolve,500);socket.addEventListener("close",()=>{clearTimeout(timer);resolve();},{once:true});}); }
...
export default async function wsActorClient(pi: ExtensionAPI) {
  ...
  registerClientHandlers(pi as any,{connect:connectEndpoint,create:withReady(createActor),list:withReady(list),resolve:withReady(resolve),send:withReady(send),ask:withReady(ask),promptStart:withReady(promptStart),promptStatus:withReady(promptObserve),promptWait:withReady(promptWait),status:withReady(status),attach:withReady(attachCommand),stop:withReady(stop),subscribe:withReady(subscribe),unsubscribe:withReady(unsubscribe),control:withReady(control)},{empty:Type.Object({}),target,actorTarget,actorTask,task,lifecycle,create:Type.Object({agentId:Type.String({minLength:1}),projectDirectory:Type.String({minLength:1}),displayName:Type.Optional(Type.String({minLength:1,maxLength:80})),role:Type.Optional(Type.String({minLength:1,maxLength:64})),nodeIdentity:Type.Optional(Type.String({minLength:1,maxLength:64}))})});
...
function actorEndpointFromTarget(target:string){const value=target.trim();if(!value)return "";if(/^wss?:\/\//.test(value))return value;if(!/^[A-Za-z0-9.-]{1,253}$/.test(value))throw new Error("actor connect target must be a clean host or websocket URL");const host=value.includes(".")?value:`${value}.taila7392b.ts.net`;return `ws://${host}:17213/actors`;}
...
function readFrame(socket:WebSocket,timeout:number):Promise<Uint8Array>{return new Promise((resolve,reject)=>{const timer=setTimeout(()=>finish(new Error("client response timeout")),timeout);const finish=(error?:Error,value?:Uint8Array)=>{clearTimeout(timer);socket.removeEventListener("message",onMessage);socket.removeEventListener("error",onError);socket.removeEventListener("close",onClose);error?reject(error):resolve(value!);};const onError=()=>finish(new Error("client daemon websocket error"));const onClose=()=>finish(new Error("client daemon disconnected"));const onMessage=async(event:MessageEvent)=>{const raw=event.data instanceof ArrayBuffer?new Uint8Array(event.data):event.data instanceof Blob?new Uint8Array(await event.data.arrayBuffer()):Buffer.isBuffer(event.data)?new Uint8Array(event.data):undefined;if(!raw||raw.byteLength<4)return finish(new Error("client response frame bound violated"));const view=Buffer.from(raw.buffer,raw.byteOffset,raw.byteLength);const length=view.readUInt32BE(0);if(!length||length>MAX_FRAME||view.byteLength<length+4)return finish(new Error("client response frame bound violated"));finish(undefined,view.subarray(4,length+4));};socket.addEventListener("message",onMessage);socket.addEventListener("error",onError,{once:true});socket.addEventListener("close",onClose,{once:true});});}
