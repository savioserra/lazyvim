...
import { REGULAR_DELIVERY_MARKER, REGULAR_DELIVERY_MESSAGE, RegularDeliveryCoordinator, RegularAckRejected, acknowledgeWithFenceRefresh, type RegularDeliveryMessage } from "./regular-delivery.ts";
...
export function frontendCompletionAckReady(presented: boolean, terminalKnown: boolean): boolean { return presented || terminalKnown; }
...
export class ActorClientConversationLog {
  ...
  terminalKnown(key: string, requestId?: string): boolean {
    const provisionalKey = requestId ? this.requestToKey.get(requestId) : undefined;
    return this.terminal.has(key) || !!(provisionalKey && this.terminal.has(provisionalKey));
  }
...
export default async function wsActorClient(pi: ExtensionAPI) {
  ...
  const reconcileConnection=async()=>{if(!context||bootstrap||!shouldReconnectClient(admin?.connected()??false,regular?.connected()??false,!!session))return;try{await ensureStarted();}catch{/* bounded timer retries while the managed daemon is unavailable */}};
  ...
  pi.on("session_start",async(_event,ctx)=>{clientGeneration++;context=ctx;const entries=ctx.sessionManager.getEntries() as any;conversation.restore(entries);projectionContext=restoreProjectionEntries(initialProjectionContext(),entries);regularDeliveries.restore(entries);if(reconnectTimer)clearInterval(reconnectTimer);reconnectTimer=setInterval(()=>{void reconcileConnection();},500);reconnectTimer.unref?.();try{await ensureStarted();}catch(error){ctx.ui.setStatus("actor-client",undefined);}});
...
export function shouldReconnectClient(adminConnected:boolean,regularConnected:boolean,hasSession:boolean):boolean{return !adminConnected||!regularConnected||!hasSession;}
...
export function buildFrontendCompletionAckRequest(value:any,frameSequence:bigint){return{completionKey:String(value.completionKey||""),frameSequence,originalRequestId:String(value.originalRequestId||""),dedupeId:String(value.dedupeId||""),chainId:String(value.chainId||""),sourceMutationSequence:typeof value.sourceMutationSequence==="bigint"?value.sourceMutationSequence:BigInt(value.sourceMutationSequence||0)}}
