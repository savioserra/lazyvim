...
import { REGULAR_DELIVERY_MARKER, REGULAR_DELIVERY_MESSAGE, RegularDeliveryCoordinator, acknowledgeWithFenceRefresh, type RegularDeliveryMessage } from "./regular-delivery.ts";
...
import { selectActorUiSnapshot, selectStatusLine } from "./projections/selectors.ts";
...
import { ActorStatusOverlay, renderActorStatusFallback } from "./widgets/actor-status-overlay.ts";
...
export type ActorClientRosterState = { connection: ProjectionContext["connection"]; epoch: bigint; sequence: bigint; agents: Map<string, ActorRosterItem>; overflow: number; render: string; degradedReason?: string; projection: ProjectionContext };
export type ActorClientRosterEvent = { type: "CONNECTING" | "CONNECTED" | "SUBSCRIBING_ROSTER" | "RECONNECTING" | "DISCONNECTED" } | { type: "ROSTER_FRAME"; frame: any };

export function initialActorClientRosterState(): ActorClientRosterState { return rosterStateFromProjection(initialProjectionContext()); }
export function reduceActorClientRoster(state: ActorClientRosterState, event: ActorClientRosterEvent): ActorClientRosterState {
  const mapped: ActorClientProjectionEvent = event.type === "CONNECTING" ? { type: "TRANSPORT.CONNECTING" } : event.type === "CONNECTED" ? { type: "TRANSPORT.CONNECTED" } : event.type === "SUBSCRIBING_ROSTER" ? { type: "TRANSPORT.SUBSCRIBING_ROSTER" } : event.type === "RECONNECTING" ? { type: "TRANSPORT.RECONNECTING" } : event.type === "DISCONNECTED" ? { type: "TRANSPORT.DISCONNECTED" } : { type: "ROSTER.FRAME", frame: event.frame };
  return rosterStateFromProjection(reduceProjection(state.projection, mapped));
}
export function renderActorClientStatus(state: ActorClientRosterState, _max = 120): string { return state.projection.snapshot.actorStatus.footer ?? ""; }
function rosterStateFromProjection(projection: ProjectionContext): ActorClientRosterState { return { connection: projection.connection, epoch: projection.roster.epoch, sequence: projection.roster.sequence, agents: projection.roster.agents, overflow: projection.snapshot.actorStatus.overflow, render: projection.snapshot.actorStatus.footer ?? "", degradedReason: projection.roster.degradedReason, projection }; }
function sanitizeStatusText(value: unknown, max: number): string | undefined { return sanitizeLabel(value, max); }
export function projectIncomingRegularDelivery(context: ProjectionContext, prompt: boolean, card: ConversationCard): { context: ProjectionContext; message: RegularDeliveryMessage } {
  const next = reduceProjection(context, { type: prompt ? "DELIVERY.INCOMING.REQUEST" : "DELIVERY.INCOMING.NOTE", key: card.key, source: { displayName: card.peerDisplayName, role: card.peerRole, authoritative: true }, body: card.body });
  const projected = next.cards.get(card.key) ?? card;
  return { context: next, message: { key: card.key, renderEnvelope: conversationEnvelope(projected) } };
}
...
export function actorReplyFramePresentationIntent(frame: { kind?: unknown }): "ask-completion" | "delivery-ack" {
  const kind = String(frame.kind ?? "").trim().toLowerCase();
  return kind === "ask" || kind === "prompt" || kind === "request" ? "ask-completion" : "delivery-ack";
}
...
export default async function wsActorClient(pi: ExtensionAPI) {
  ...
  const fences=new Map<string,Fence>();const peerCache=new Map<string,PeerMetadata>();let sourcePeer:PeerMetadata|undefined;const mutations=new ClientMutationSequencer();let projectionContext:ProjectionContext=initialProjectionContext();const projectionListeners=new Set<()=>void>();
  const publishProjection=()=>{context?.ui.setStatus("actor-client",selectStatusLine(projectionContext));for(const listener of projectionListeners)listener();};
  const applyProjectionEvent=(event:ActorClientProjectionEvent)=>{projectionContext=reduceProjection(projectionContext,event);publishProjection();};
  const applyRosterEvent=(event:ActorClientRosterEvent)=>{const mapped:ActorClientProjectionEvent=event.type==="CONNECTING"?{type:"TRANSPORT.CONNECTING"}:event.type==="CONNECTED"?{type:"TRANSPORT.CONNECTED"}:event.type==="SUBSCRIBING_ROSTER"?{type:"TRANSPORT.SUBSCRIBING_ROSTER"}:event.type==="RECONNECTING"?{type:"TRANSPORT.RECONNECTING"}:event.type==="DISCONNECTED"?{type:"TRANSPORT.DISCONNECTED"}:{type:"ROSTER.FRAME",frame:event.frame};applyProjectionEvent(mapped);};
  const setPendingAskStatus=(_count:number)=>publishProjection();
  const applyActivityFrames=(frames:readonly any[]|undefined)=>{for(const frame of frames??[])applyProjectionEvent({type:"ACTIVITY.FRAME",frame});};
  const regularDeliveries=new RegularDeliveryCoordinator({appendMarker:(marker)=>pi.appendEntry(REGULAR_DELIVERY_MARKER,marker),projectIncoming:(_delivery,prompt,card)=>{const projected=projectIncomingRegularDelivery(projectionContext,prompt,card);projectionContext=projected.context;publishProjection();return projected.message;},sendFollowUp:(message,text)=>pi.sendMessage({customType:REGULAR_DELIVERY_MESSAGE,content:text,display:true,details:message},{deliverAs:"followUp",triggerTurn:true}),acknowledge:async(delivery,fence,delivered,answer,reason)=>{const current=required(session);const request={agentId:delivery.targetAgentId||current.caller,sequence:delivery.sequence,dedupeId:delivery.dedupeId,delivered,reason,boundedResult:new TextEncoder().encode(answer),kind:delivery.kind===1?"notification":"prompt",sourceScope:delivery.sourceScope,completionKey:delivery.completionKey};const attempt=async(activeFence:Fence)=>{const response=await required(regular).request("bridgeDeliveryAckRequest",BridgeDeliveryAckRequestSchema,request,{fence:activeFence,requestId:randomUUID(),timeout:NORMAL_TIMEOUT});if(response.payload.case!=="bridgeDeliveryAckResponse"||!response.payload.value.accepted)throw new Error(response.payload.value?.reason||"regular delivery acknowledgement rejected");};await acknowledgeWithFenceRefresh(fence,attempt,async()=>{fences.delete(current.caller);return attach(current.caller);});}});
  const handlePush=(envelope:Envelope)=>{if(envelope.payload?.case==="clientAgentRosterFrame")applyRosterEvent({type:"ROSTER_FRAME",frame:envelope.payload.value});if(envelope.payload?.case==="agentActivityFrame")applyProjectionEvent({type:"ACTIVITY.FRAME",frame:envelope.payload.value});if(envelope.payload?.case==="bridgePushFrame"){applyActivityFrames(envelope.payload.value.activityFrames);const fence={handle:String(envelope.agentHandle||""),fence:BigInt(envelope.agentFence??0)};for(const delivery of envelope.payload.value.deliveries??[])void regularDeliveries.deliver(delivery,fence).catch((error)=>context?.ui.setStatus("actor-client",`actor delivery degraded · ${safePeerText(error instanceof Error?error.message:String(error),100)}`));}if(envelope.payload?.case==="actorMessageReplyFrame"){void (async()=>{const value=envelope.payload.value;if(actorReplyFramePresentationIntent(value)!=="ask-completion"){if(session)mutations.adoptHighWater(clientMessageScopeKey(session.caller),typeof value.sourceMutationSequence==="bigint"?value.sourceMutationSequence:0n);return;}const requestId=String(value.originalRequestId||envelope.requestId||"");const key=value.completionKey?String(value.completionKey):`actor-client:${requestId}`;const completion={type:"TASK.COMPLETED" as const,key,reply:new TextDecoder().decode(value.boundedResult??new Uint8Array()),completed:!!value.completed,reason:value.reason,source:peerFromProto(value.source),target:peerFromProto(value.target),kind:value.kind,requestId,dedupeId:value.dedupeId,chainId:value.chainId,sourceMutationSequence:String(value.sourceMutationSequence??"")};try{const presented=await conversation.complete(completion);if(presented){applyProjectionEvent(completion);const digest=projectionContext.cards.get(key)?.terminalDigest??digestPresentation(completion);applyProjectionEvent({type:"PRESENTATION.SUCCEEDED",key,digest});}}catch(error){const reason=safePeerText(error instanceof Error?error.message:String(error),120)||"actor completion presentation failed";applyProjectionEvent({type:"PRESENTATION.FAILED",key,reason});}if(session)mutations.adoptHighWater(clientMessageScopeKey(session.caller),typeof value.sourceMutationSequence==="bigint"?value.sourceMutationSequence:0n);})().catch((error)=>context?.ui.setStatus("actor-client",`actors degraded · ${safePeerText(error instanceof Error?error.message:String(error),100)}`));}};
  ...
  pi.registerMessageRenderer<RegularDeliveryMessage>(REGULAR_DELIVERY_MESSAGE,(message,_options,theme)=>renderActorClientConversationEnvelope((message.details as RegularDeliveryMessage).renderEnvelope ?? envelopeFromLegacy({ communicationView: (message.details as RegularDeliveryMessage).view }),theme));
  pi.registerCommand("actor-status",{description:"Open a read-only actor status overlay",handler:async(_args,ctx)=>{const snapshot=()=>selectActorUiSnapshot(projectionContext);if(ctx.mode!=="tui"){ctx.ui.notify(renderActorStatusFallback(snapshot()),"info");return;}if(typeof ctx.ui.custom!=="function")return;await ctx.ui.custom<void>((tui:any,theme:any,_keybindings:any,done:(value:void)=>void)=>{let overlay:ActorStatusOverlay;const listener=()=>{overlay.invalidate();tui.requestRender();};overlay=new ActorStatusOverlay(snapshot,theme,()=>done());projectionListeners.add(listener);return{render:(width:number)=>overlay.render(width),invalidate:()=>overlay.invalidate(),handleInput:(data:string)=>{overlay.handleInput(data);tui.requestRender();},dispose:()=>projectionListeners.delete(listener)};},{overlay:true});}});
  ...
  const status=async(agentId:string)=>{validateAgentID(agentId);const cached=projectionContext.roster.agents.get(agentId);if(cached)return {accepted:true,agentId:cached.agentId,displayName:cached.displayName,role:cached.role,lifecycle:cached.lifecycle,revision:String(cached.revision)};const resolved=await resolve(agentId);const found=resolved?.agent;if(found?.agentId)return cachePeer(found);return {accepted:false,agentId,reason:"agent not found or access denied"};};
...
function terminalMetadata(value:string|undefined,max:number){const trimmed=value?.trim()??"";if(!trimmed)return "";if(trimmed.length>max||/[\0\r\n\t]/.test(trimmed))throw new Error("terminal metadata is invalid");return trimmed;}
