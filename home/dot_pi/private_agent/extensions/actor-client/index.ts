...
import { reduceProjection, initialProjectionContext } from "./projections/machine.ts";
...
import type { ActorRosterItem, PeerMetadata, ProjectionContext } from "./projections/types.ts";
...
export class ActorClientConversationLog {
  ...
  async complete(entry: { key: string; reply: string; completed: boolean; reason?: string; source?: PeerMetadata; target?: PeerMetadata; kind?: string; requestId?: string; dedupeId?: string; chainId?: string; sourceMutationSequence?: string }): Promise<boolean> {
    if (this.terminal.has(entry.key) || this.presenting.has(entry.key)) return false;
    this.presenting.add(entry.key);
    ...
    try {
      ...
      this.presenting.delete(entry.key);
      throw error;
    }
    this.presenting.delete(entry.key); this.terminal.add(entry.key); this.seenEntries.add(entry.key); this.pending.delete(entry.key); this.updateStatus();
    return true;
  }
...
export default async function wsActorClient(pi: ExtensionAPI) {
  ...
  const setPendingAskStatus=(count:number)=>{context?.ui.setStatus("actor-client-asks",count?`actor asks ${count} pending`:undefined);projectionContext=reduceProjection(projectionContext,{type:"VIEW.WIDTH",width:projectionContext.snapshot.width});};
  ...
  const handlePush=(envelope:Envelope)=>{if(envelope.payload?.case==="clientAgentRosterFrame")applyRosterEvent({type:"ROSTER_FRAME",frame:envelope.payload.value});if(envelope.payload?.case==="bridgePushFrame"){const fence={handle:String(envelope.agentHandle||""),fence:BigInt(envelope.agentFence??0)};for(const delivery of envelope.payload.value.deliveries??[])void regularDeliveries.deliver(delivery,fence).catch((error)=>context?.ui.setStatus("actor-client",`actor delivery degraded · ${safePeerText(error instanceof Error?error.message:String(error),100)}`));}if(envelope.payload?.case==="actorMessageReplyFrame"){const value=envelope.payload.value;const key=`actor-client:${value.completionKey||value.originalRequestId||envelope.requestId}`;const completion={type:"TASK.COMPLETED" as const,key,reply:new TextDecoder().decode(value.boundedResult??new Uint8Array()),completed:!!value.completed,reason:value.reason,source:peerFromProto(value.source),target:peerFromProto(value.target),kind:value.kind,requestId:value.originalRequestId||envelope.requestId,dedupeId:value.dedupeId,chainId:value.chainId,sourceMutationSequence:String(value.sourceMutationSequence??"")};projectionContext=reduceProjection(projectionContext,completion);void conversation.complete(completion);if(session)mutations.adoptHighWater(clientMessageScopeKey(session.caller),typeof value.sourceMutationSequence==="bigint"?value.sourceMutationSequence:0n);}};
  ...
  pi.registerMessageRenderer<RegularDeliveryMessage>(REGULAR_DELIVERY_MESSAGE,(message,_options,theme)=>renderCommunicationCard((message.details as RegularDeliveryMessage).view,theme));
  ...
  pi.on("session_start",async(_event,ctx)=>{clientGeneration++;context=ctx;const entries=ctx.sessionManager.getEntries() as any;conversation.restore(entries);regularDeliveries.restore(entries);try{await ensureStarted();}catch(error){ctx.ui.setStatus("actor-client",undefined);}});
