...
local function restore_locked(lock)
	...
	if not completed then
		...
		if not terminated then
			error(
				"Mason restore could not terminate timed-out operations: " .. table.concat(sorted_keys(pending), ", ")
			)
		end
	end
	...
	for package_name, package_version in pairs(lock_data) do
		...
		if ok and not failures[package_name] then
			...
			if installed_version ~= package_version then
				failures[package_name] = ("expected %s, got %s"):format(
					package_version,
